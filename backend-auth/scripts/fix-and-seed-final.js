...
async function seedDatabase() {
  try {
    ...
    try {
      await pool.query('ALTER TABLE users_background ADD CONSTRAINT users_background_pkey PRIMARY KEY (user_id);');
      console.log('Primary key constraint added');
    } catch (constraintError) {
      console.log('Primary key constraint may already exist:', constraintError.message);
    }
    ...
    // Now insert test data (without ON CONFLICT since we just created the constraint)
